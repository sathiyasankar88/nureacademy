...
class Slider implements Lib\ShortcodeInterface {
	...
	public function render( $atts, $content = null ) {
		...
		if ( $slider != "" ) {
			...
			if ( $target_qodef_options['smooth_page_transitions'] == "yes" && $target_qodef_options['smooth_pt_spinner_type'] != "" ) {
				$ajax_loader = '<div class="qodef-st-loader"><div class="qodef-st-loader1">' . target_qodef_loading_spinners( true ) . '</div></div>';
			} else {
				$ajax_loader = '<div class="qodef-st-loader"><div class="qodef-st-loader1">' . target_qodef_loading_spinner_pulse() . '</div></div>';
			}

			/* set slider preloader - end */

			/* set padding for slider arrows - start */
			$slider_arrows_padding = "padding-top: " . esc_attr( $this->target_qodef_get_slider_navigation_padding() ) . "px;";
			/* set padding for slider arrows - end */

			$html .= '<div class="qodef-slider">';
			$html .= '<div class="qodef-slider-inner">';
			$html .= '<div id="qodef-' . esc_attr( $slider ) . '" ' . $slider_anchor . ' class="carousel slide ' . esc_attr( $animation_type_class . ' ' . $full_screen_class . ' ' . $responsive_height_class . ' ' . $height_class . ' ' . $auto_start_class . ' ' . $header_effect_class . ' ' . $slider_numbers_class . ' ' . $slider_thumbs_class ) . ' " ' . $slide_animation_timeout . ' ' . $data_height . ' ' . $data_parallax_effect . ' ' . target_qodef_get_inline_style( $slide_holder_height ) . '><div class="qodef-slider-preloader">' . $ajax_loader . '</div>';
			$html .= '<div class="carousel-inner ' . esc_attr( $slider_css_position_class ) . '" ' . target_qodef_get_inline_style( $carouselinner_height ) . ' ' . $data_parallax_transform . '>';
			...
			if ( have_posts() ) : $postCount = 0;
				while ( have_posts() ) : the_post();
					...
					if ( $slide_type == 'video' ) {
						...
						$html .= '<div class="qodef-video"><div class="qodef-mobile-video-image" ' . target_qodef_get_inline_style( 'background-image: url(' . esc_url( $video_image ) . ')' ) . '></div><div class="qodef-video-overlay';
						...
						$html .= '>';
						if ( $video_overlay_image != "" ) {
							$html .= '<img src="' . esc_url( $video_overlay_image ) . '" alt="' . esc_attr__( 'Video image', 'select-core' ) . '" />';
						} else {
							$html .= '<img src="' . esc_url( get_template_directory_uri() ) . '/assets/css/img/pixel-video.png" alt="' . esc_attr__( 'Pixel image', 'select-core' ) . '" />';
						}
						...
						$html .= '<object width="320" height="240" type="application/x-shockwave-flash" data="' . esc_url( get_template_directory_uri() ) . '/assets/js/flashmediaelement.swf">
                                                    ...
                                                    <img src="' . esc_url( $video_image ) . '" width="1920" height="800" title="' . esc_attr__( 'No video playback capabilities', 'select-core' ) . '" alt="' . esc_html__( 'Video thumb', 'select-core' ) . '" />
                                            </object>
                                    </video>
                            </div></div>';
					} else {
						$html .= '<div class="qodef-image" ' . target_qodef_get_inline_style( 'background-image:url(' . esc_url( $image ) . ')' ) . '>';
						if ( $slider_thumbs == 'no' ) {
							$html .= '<img src="' . esc_url( $image ) . '" alt="' . esc_attr( $title ) . '">';
						}

						if ( $image_overlay_pattern !== "" ) {
							$html .= '<div class="qodef-image-pattern" ' . target_qodef_get_inline_style( 'background: url(' . esc_url( $image_overlay_pattern ) . ') repeat 0 0' ) . '></div>';
						}
						$html .= '</div>';
					}
					...
					if ( $element_positions == 'custom' ) {
						$html .=
							'<div class="qodef-slider-elements-container qodef-custom-elements ' . ( $frame_in_grid == 'yes' ? 'qodef-grid' : '' ) . '" ' . target_qodef_get_inline_style( ( $frame_in_grid == 'no' && ( ( $meta_temp = get_post_meta( get_the_ID(), "qodef_slide_holder_frame_width", true ) ) != '' ) ? 'width: ' . $meta_temp . '%; ' : '' ) . 'padding-top: ' . esc_attr( $this->target_qodef_get_slider_navigation_padding() ) . 'px;' ) . ' ' . $slide_elements_data_start . ' ' . $slide_elements_data_end . ( $frame_in_grid == 'yes' ? '' : ' data-width-mobile="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_mobile', true ) ) . '" data-width-tablet-p="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_tablet_p', true ) ) . '" data-width-tablet-l="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_tablet_l', true ) ) . '" data-width-laptop="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_laptop', true ) ) . '" data-width-desktop="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_desktop', true ) ) . '"' ) . '>' .
							'<div class="qodef-slider-elements-container-inner" ' . target_qodef_get_inline_style( ( ( $meta_temp = get_post_meta( get_the_ID(), "qodef_slide_holder_frame_height", true ) ) != '' ) ? 'padding-bottom: ' . $meta_temp . '%;' : '' ) . '">' .
							'<div class="qodef-slider-elements-holder-frame" ' . target_qodef_get_inline_style( 'width: 100%; height: 100%' ) . ' data-default-width="' . esc_attr( $def_width ) . '">';
					} else {
						$html .=
							'<div class="qodef-slider-elements-container" ' . target_qodef_get_inline_style( 'width: 100%; box-sizing: border-box; height: 100%; ' . 'padding-top: ' . esc_attr( $this->target_qodef_get_slider_navigation_padding() ) . 'px;' ) . ' ' . $slide_elements_data_start . ' ' . $slide_elements_data_end . ( $frame_in_grid == 'yes' ? '' : ' data-width-mobile="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_mobile', true ) ) . '" data-width-tablet-p="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_tablet_p', true ) ) . '" data-width-tablet-l="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_tablet_l', true ) ) . '" data-width-laptop="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_laptop', true ) ) . '" data-width-desktop="' . esc_attr( get_post_meta( get_the_ID(), 'qodef_slide_holder_frame_width_desktop', true ) ) . '"' ) . '>' .
							'<div class="qodef-slider-elements-container-inner" ' . target_qodef_get_inline_style( 'height: 100%;' ) . '>' .
							'<div class="qodef-slider-elements-holder-frame ' . ( $frame_in_grid == 'yes' ? 'qodef-grid' : '' ) . '" ' . target_qodef_get_inline_style( ( $frame_in_grid == 'no' && ( ( $meta_temp = get_post_meta( get_the_ID(), "qodef_slide_holder_frame_width", true ) ) != '' ) ? 'width: ' . $meta_temp . '%; ' : '' ) . 'top: 50%; left: 50%; -webkit-transform: translate(-50%,-50%); transform: translate(-50%,-50%);' ) . ' data-default-width="' . esc_attr( $def_width ) . '">';
					}
								...
								case 'text':
									...
									if ( $text_options == 'advanced' ) {
										...
										if ( $slide_element['slideelementtextbgndcolor'] != '' ) {
											...
											$style_string .= 'background-color: ' . target_qodef_rgba_color( $slide_element_bg_color, $slide_element_bg_transparency ) . ';';
										}
								...
								case 'button':
									...
									if ( $slide_element['slideelementbuttoniconpack'] != '' ) {
										...
										if ( $button_params['icon_pack'] != 'no_icon' ) {
											$iconPackName = target_qodef_icon_collections()->getIconCollectionParamNameByKey( $button_params['icon_pack'] );
									...
									$slide_element_html .= '<div class="' . $class_string . '" ' . $data_string . ' style="' . $style_string . '"><div class="qodef-slide-element-inner" style="' . $inner_style_string . '">' . target_qodef_get_button_html( $button_params ) . '</div></div>';
									break;
				...
				if ( $show_navigation_circles == "yes" ) {
					...
					if ( have_posts() ) : $postCount = 0;
						while ( have_posts() ) : the_post();
							...
							$slide_type = get_post_meta( get_the_ID(), "qodef_slide_background_type", true );
							if ( $slide_type == 'image' ) {
								$img .= target_qodef_generate_thumbnail( '', esc_url( get_post_meta( get_the_ID(), "qodef_slide_image", true ) ), '160', '100' );
							} elseif ( $slide_type == 'video' ) {
								$img .= target_qodef_generate_thumbnail( '', esc_url( get_post_meta( get_the_ID(), "qodef_slide_video_image", true ) ), '160', '100' );
							}
							$html .= '>';
							$html .= $img;
							$html .= '</li>';
	 ...
	 **/
	function target_qodef_get_slider_item_padding() {
		return apply_filters( 'target_qodef_slider_item_padding', 0 );
	}

	/**
	 * Function that returns slider navigation pading
	 **/
	function target_qodef_get_slider_navigation_padding() {
		return apply_filters( 'target_qodef_slider_navigation_padding', 0 );
	}
}