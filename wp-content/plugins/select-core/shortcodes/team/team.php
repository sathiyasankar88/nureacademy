<?php
namespace TargetQodef\Modules\Shortcodes\Team;
...
class Team implements ShortcodeInterface
{
	...
	public function vcMap()	{
		...
		for ($x = 1; $x<6; $x++) {
			$teamIconCollections = target_qodef_icon_collections()->getCollectionsWithSocialIcons();
			...
			$team_social_icons_array[] = array(
				...
				'dependency' => array('element' => 'team_social_icon_pack', 'value' => target_qodef_icon_collections()->getIconCollectionsKeys())
			);
			...
			'params' => array_merge(
				array(
					...
					array(
						...
						'admin_label' => true,
						'value' => array_merge(array('' => ''),target_qodef_icon_collections()->getIconCollectionsVCExclude(array('dripicons', 'linea_icons','linear_icons'))),
						'save_always' => true
					),
					array(
						'type' => 'dropdown',
	...
	private function getTeamSocialIcons($params) {
		...
		if ($team_social_icon_pack !== '') {
			...
			$icon_pack = target_qodef_icon_collections()->getIconCollection($team_social_icon_pack);
			...
			for ( $i = 1; $i <= $number_of_social_icons; $i++ ) {
				...
				if ($team_social_icon !== '') {
					...
					$social_icons[] = target_qodef_execute_shortcode('qodef_icon', $team_icon_params);
				}
