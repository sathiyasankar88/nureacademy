<?php
namespace TargetQodef\Modules\Shortcodes\SocialShare;
...
class SocialShare implements ShortcodeInterface {
	...
	public function render($atts, $content = null) {
		...
		$params['enable_social_share'] = (target_qodef_options()->getOptionValue('enable_social_share') == 'yes') ? true : false;

		//Is social share enabled for post type
		$post_type = get_post_type();
		$params['enabled'] = (target_qodef_options()->getOptionValue('enable_social_share_on_'.$post_type) == 'yes') ? true : false;
	...
	private function getSocialNetworksParams($params) {
		...
		foreach ($this->socialNetworks as $net) {
			...
			$html = '';
			if (target_qodef_options()->getOptionValue('enable_'.$net.'_share') == 'yes') {
				...
				$params['custom_icon'] = (target_qodef_options()->getOptionValue($net.'_icon')) ? target_qodef_options()->getOptionValue($net.'_icon') : '';
	...
	private function getSocialNetworkShareLink($net, $image) {
		...
		switch ($net) {
			...
			case 'twitter':
				...
				$twitter_via = (target_qodef_options()->getOptionValue('twitter_via') !== '') ? ' via ' . target_qodef_options()->getOptionValue('twitter_via') . ' ' : '';
				
				if(wp_is_mobile()) {
					$link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(target_qodef_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
				} else {
					$link = 'window.open(\'http://twitter.com/home?status=' . urlencode(target_qodef_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
				}
				break;
			case 'google_plus':
