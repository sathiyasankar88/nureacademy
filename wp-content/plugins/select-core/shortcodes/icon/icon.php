<?php
namespace TargetQodef\Modules\Shortcodes\Icon;

use TargetQodef\Modules\Shortcodes\Lib\ShortcodeInterface;

/**
 * Class Icon
 * @package TargetQodef\Modules\Shortcodes\Icon
 */
class Icon implements ShortcodeInterface {
            ...
            'params'                    => array_merge(
                ...
                array(
                    ...
                    array(
                        ...
                        'value'       => array(esc_html__('Use this icon as Anchor?', 'select-core') => 'yes'),
        ...
        if(($params['type'] !== 'normal' && !empty($params['shape_size'])) ||
           ($params['type'] == 'normal')
            ...
            if(!empty($params['custom_size'])) {
                $iconStyles[] = 'font-size:'.target_qodef_filter_px($params['custom_size']).'px';
            }
        }
    ...
    private function generateIconHolderStyles($params) {
        ...
        if(isset($params['type']) && $params['type'] !== 'normal') {
            ...
            if(!empty($shapeSize)) {
                $iconHolderStyles[] = 'width: '.target_qodef_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'height: '.target_qodef_filter_px($shapeSize).'px';
                $iconHolderStyles[] = 'line-height: '.target_qodef_filter_px($shapeSize).'px';
            }
            ...
            if(!empty($params['border_color']) && (isset($params['border_width']) && $params['border_width'] !== '')) {
				...
				$iconHolderStyles[] = 'border-width: '.target_qodef_filter_px($params['border_width']).'px';
            ...
            if($params['type'] == 'square') {
                if(isset($params['border_radius']) && $params['border_radius'] !== '') {
                    $iconHolderStyles[] = 'border-radius: '.target_qodef_filter_px($params['border_radius']).'px';
                }
            }
        }
