...
class TargetQodefInstagramWidget extends TargetQodefWidget {
	protected $params;
		...
		$this->params = array(
			...
			array(
				...
				'options' => array(
					...
					'6' => esc_html__( 'Six', 'select-core' ),
					'9' => esc_html__( 'Nine', 'select-core' )
				)
			),
			array(
				'name'    => 'image_size',
				...
				foreach ( $images_array as $image ) { ?>
                    <li>
                        <a itemprop="url" href="<?php echo esc_url( $instagram_api->getHelper()->getImageLink( $image ) ); ?>" target="_blank">
							<?php echo target_qodef_kses_img( $instagram_api->getHelper()->getImageHTML( $image, $image_size ) ); ?>
                        </a>
                    </li>
				<?php } ?>
            </ul>
