	...
	function generate_body_code($id, $preview) {
		...
		if ($item_row != null)
		{
			...
			else
			{
				...
				// google analytics
				if ($data->enablegoogleanalytics) {
					
					$dataoptions .= ' data-enablegoogleanalytics=1';

					if (!empty($data->ga4account)) {
						$dataoptions .= ' data-ga4account="' . esc_html($data->ga4account) . '"';
					}

					if (!empty($data->gaid)) {
						$dataoptions .= ' data-gaid="' . esc_html($data->gaid) . '"';
					}

					$dataoptions .= ' data-gaeventcategory="' . esc_html($data->gaeventcategory) . '" data-gaeventlabel="' . esc_html($data->gaeventlabel) . '"';
				}
