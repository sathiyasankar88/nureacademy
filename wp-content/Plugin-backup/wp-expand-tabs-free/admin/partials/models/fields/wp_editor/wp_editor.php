...
if ( ! class_exists( 'SP_WP_TABS_Field_wp_editor' ) ) {
	...
	class SP_WP_TABS_Field_wp_editor extends SP_WP_TABS_Fields {
      ...
      // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo '<textarea name="' . esc_attr( $this->field_name() ) . '"' . $this->field_attributes( $attributes ) . $editor_height . '>' . wp_kses( nl2br( htmlentities( $this->value, ENT_QUOTES, 'UTF-8' ) ), apply_filters( 'sp_tabs_description_allow_tags', $allowed_tags ) ) . '</textarea>';
