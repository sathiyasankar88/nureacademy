 ...
 */
if ( ! class_exists( 'TargetQodefStickyNavigationWalker' ) ) {
	class TargetQodefStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			//menu type class
			$wide_background = '';
			if ( $target_qodef_options['enable_wide_menu_background'] == "yes" ) {
			...
			if ( $item->hide == "" ) {
				...
				if ( $item->featured_icon !== "" && $item->featured_icon == "Star" ) {
					$item_output .= '<span class="qodef-featured-icon star" aria-hidden="true">' . esc_html__( "*", "targetwp" ) . '</span>';
				} else if ( $item->featured_icon !== "" && $item->featured_icon == "New" ) {
					$item_output .= '<span class="qodef-featured-icon text" aria-hidden="true">' . esc_html__( "New", "targetwp" ) . '</span>';
				}
