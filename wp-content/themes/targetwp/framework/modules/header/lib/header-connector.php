<?php
namespace TargetQodef\Modules\Header\Lib;
/**
...
class HeaderConnector {
    ...
    public function connect($config = array()) {
        do_action('target_qodef_pre_header_connect');
        ...
        if(!empty($config['affect_content'])) {
            add_filter('target_qodef_content_elem_style_attr', array($this, 'contentMarginFilter'));
        }

        if(!empty($config['affect_title'])) {
            add_filter('target_qodef_title_content_padding', array($this, 'titlePaddingFilter'));
        }

        if(!empty($config['affect_slider'])) {

            add_filter('target_qodef_slider_item_padding', array($this, 'sliderItemPaddingFilter'));
            add_filter('target_qodef_slider_navigation_padding', array($this, 'sliderNavigationPaddingFilter'));
        }

        do_action('target_qodef_after_header_connect');
    }
     ...
     *
     * Hooks to target_qodef_title_content_padding filter
     *
     * @return int
     */
     ...
     *
     * Hooks to target_qodef_slider_item_padding filter
     *
     * @return int
     */
