...
if ( ! function_exists( 'target_qodef_get_title' ) ) {
	/**
	 * Loads title area HTML
	 */
	function target_qodef_get_title() {
		$id = target_qodef_get_page_id();

		extract( target_qodef_title_area_height() );
		extract( target_qodef_title_area_background() );
		...
		if ( ( $meta_temp = get_post_meta( $id, "qodef_show_title_area_meta", true ) ) !== "" ) {
			...
			$show_title_area = target_qodef_options()->getOptionValue( 'show_title_area' ) == 'yes' ? true : false;
		}
		...
		if ( ( $meta_temp = get_post_meta( $id, "qodef_title_area_type_meta", true ) ) !== "" ) {
			...
			$type = target_qodef_options()->getOptionValue( 'title_area_type' );
		}
		...
		if ( ( $meta_temp = get_post_meta( $id, "qodef_title_area_enable_breadcrumbs_meta", true ) ) !== "" ) {
			...
			$enable_breadcrumbs = target_qodef_options()->getOptionValue( 'title_area_enable_breadcrumbs' ) == 'yes' ? true : false;
		}
		...
		$separator_color = '';
		if ( ( $meta_temp = get_post_meta( $id, "qodef_separator_color_meta", true ) ) !== "" ) {
			$separator_color = 'background-color:' . $meta_temp . ';';
		} else {
			$separator_color = 'background-color:' . target_qodef_options()->getOptionValue( 'page_separator_color' );
		}
		...
		$parameters = apply_filters( 'target_qodef_title_area_height_params', $parameters );

		target_qodef_get_module_template_part( 'templates/title', 'title', '', $parameters );
	}
}

if ( ! function_exists( 'target_qodef_get_title_text' ) ) {
	/**
	 * Function that returns current page title text. Defines target_qodef_title_text filter
	 ...
	 */
	function target_qodef_get_title_text() {
		...
		if ( is_tag() ) {
			...
			$title = single_term_title( "", false ) . esc_html__( ' Tag', 'targetwp' );
		...
		elseif ( is_author() ) {
			...
			$title = esc_html__( 'Author:', 'targetwp' ) . " " . get_the_author();
		...
		elseif ( is_singular( 'post' ) ) {
			if ( target_qodef_options()->getOptionValue( 'blog_single_title_in_title_area' ) == "yes" ) {
		...
		elseif ( is_404() ) {
			...
			if ( target_qodef_options()->getOptionValue( '404_title' ) != "" ) {
				//get it from options
				$title = target_qodef_options()->getOptionValue( '404_title' );
			} else {
				//get default 404 page title
				$title = esc_html__( '404 - Page not found', 'targetwp' );
			}
		} //is WooCommerce installed and is shop or single product page?
		elseif ( target_qodef_is_woocommerce_installed() && ( is_shop() || is_singular( 'product' ) ) ) {
		...
		elseif ( target_qodef_is_woocommerce_installed() && ( is_product_category() || is_product_tag() ) ) {
			global $wp_query;
		...
		elseif ( is_archive() ) {
			$title = esc_html__( 'Archive', 'targetwp' );
		} //current page is regular page
		else {
			$title = get_the_title( $id );
		}

		$title = apply_filters( 'target_qodef_title_text', $title );

		return $title;
	}
}

if ( ! function_exists( 'target_qodef_title_text' ) ) {
	/**
	 * Function that echoes title text.
	 *
	 * @see target_qodef_get_title_text()
	 */
	function target_qodef_title_text() {
		echo target_qodef_get_title_text();
	}
}

if ( ! function_exists( 'target_qodef_subtitle_text' ) ) {
	/**
	 * Function that echoes subtitle text.
	 *
	 */
	function target_qodef_subtitle_text() {
		$id       = target_qodef_get_page_id();
		$subtitle = '';
		...
		print target_qodef_get_module_part( $subtitle );
	}
}

if ( ! function_exists( 'target_qodef_custom_breadcrumbs' ) ) {
	/**
	 ...
	 * @see is_front_page()
	 * @see is_category()
	 * @see target_qodef_is_product_category()
	 * @see get_search_query()
	 */
	function target_qodef_custom_breadcrumbs() {
		global $post;

		$output      = "";
		$homeLink    = esc_url( home_url( '/' ) );
		$pageid      = target_qodef_get_page_id();
		$bread_style = "";
		...
		$delimiter   = '<span class="qodef-delimiter" ' . target_qodef_get_inline_style( $bread_style ) . '>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
		$home        = esc_html__( 'Home', 'targetwp' ); // text for the 'Home' link
		$showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
		$before      = '<span class="qodef-current" ' . target_qodef_get_inline_style( $bread_style ) . '>'; // tag before the current crumb
		$after       = '</span>'; // tag after the current crumb

		if ( is_home() && ! is_front_page() ) {
			$output = '<div class="qodef-breadcrumbs"><div class="qodef-breadcrumbs-inner"><a itemprop="url" ' . target_qodef_get_inline_style( $bread_style ) . ' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a itemprop="url" ' . target_qodef_get_inline_style( $bread_style ) . ' href="' . $homeLink . '">' . get_the_title( $pageid ) . '</a></div></div>';

		} elseif ( is_home() ) {
			$output = '<div class="qodef-breadcrumbs"><div class="qodef-breadcrumbs-inner">' . $before . $home . $after . '</div></div>';
		} elseif ( is_front_page() ) {
			if ( $showOnHome == 1 ) {
				$output = '<div class="qodef-breadcrumbs"><div class="qodef-breadcrumbs-inner"><a itemprop="url" ' . target_qodef_get_inline_style( $bread_style ) . ' href="' . $homeLink . '">' . $home . '</a></div></div>';
			}
		} else {

			$output .= '<div class="qodef-breadcrumbs"><div class="qodef-breadcrumbs-inner"><a itemprop="url" ' . target_qodef_get_inline_style( $bread_style ) . ' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

			if ( is_category() || target_qodef_is_product_category() ) {
				...
				$output .= $before . esc_html__( 'Search results for ', 'targetwp' ) . '"' . get_search_query() . '"' . $after;

			} elseif ( is_day() ) {
				$output .= '<a itemprop="url" ' . target_qodef_get_inline_style( $bread_style ) . ' href="' . get_year_link( get_the_time( 'Y' ) ) . '">' . get_the_time( 'Y' ) . '</a>' . $delimiter;
				$output .= '<a itemprop="url" ' . target_qodef_get_inline_style( $bread_style ) . ' href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '">' . get_the_time( 'F' ) . '</a>' . $delimiter;
				...
				if ( get_post_type() != 'post' ) {
					...
					$output .= '<span class="qodef-breadcrumb-cat-holder" ' . target_qodef_get_inline_style( $bread_style ) . '>' . $cats . '</span>';
					if ( $showCurrent == 1 ) {
						$output .= $before . get_the_title() . $after;
					}
				}
				...
				$output .= '<a itemprop="url" ' . target_qodef_get_inline_style( $bread_style ) . ' href="' . get_permalink( $parent ) . '">' . $parent->post_title . '</a>';
				if ( $showCurrent == 1 ) {
				...
				while ( $parent_id ) {
					...
					$breadcrumbs[] = '<a itemprop="url" ' . target_qodef_get_inline_style( $bread_style ) . ' href="' . get_permalink( $page->ID ) . '">' . get_the_title( $page->ID ) . '</a>';
				...
				$output .= $before . esc_html__( 'Posts tagged ', 'targetwp' ) . '"' . single_tag_title( '', false ) . '"' . $after;

			} elseif ( is_author() ) {
				global $authordata;
				$output .= $before . esc_html__( 'Articles posted by ', 'targetwp' ) . $authordata->display_name . $after;
			...
			if ( get_query_var( 'paged' ) ) {
				...
				$output .= $before . " (" . esc_html__( 'Page', 'targetwp' ) . ' ' . get_query_var( 'paged' ) . ")" . $after;
...
if ( ! function_exists( 'target_qodef_get_title_area_height' ) ) {

	/**
	 * Function that returns title height
	 **/
	function target_qodef_get_title_area_height() {
		...
		if ( ( $meta_temp = get_post_meta( $id, 'qodef_title_area_type_meta', true ) ) != '' ) {
			...
			$title_type = target_qodef_options()->getOptionValue( 'title_area_type' );
		}
		...
		} elseif ( target_qodef_options()->getOptionValue( 'title_area_height' ) !== '' ) {
			$title_height = target_qodef_options()->getOptionValue( 'title_area_height' );
		}

		return apply_filters( 'target_qodef_title_area_height', $title_height );
	}
}

if ( ! function_exists( 'target_qodef_get_title_content_padding' ) ) {
	/**
	 * Function that returns title content pading
	 **/

	function target_qodef_get_title_content_padding() {
		$title_padding = apply_filters( 'target_qodef_get_title_content_padding', 0 );
...
if ( ! function_exists( 'target_qodef_title_area_height' ) ) {
	/**
	 * Function that returns title height and padding to be applied in template
	 **/

	function target_qodef_title_area_height() {
		$id                            = target_qodef_get_page_id();
		$title_height_and_padding      = array();
		$title_height                  = target_qodef_get_title_area_height();
		$header_height_padding         = target_qodef_get_title_content_padding();
		...
		if ( ( $meta_temp = get_post_meta( $id, "qodef_title_area_background_image_responsive_meta", true ) ) != "" ) {
			...
			$is_img_responsive = target_qodef_options()->getOptionValue( 'title_area_background_image_responsive' );
		}

		if ( ( $meta_temp = get_post_meta( $id, "qodef_title_area_vertial_alignment_meta", true ) ) !== '' ) {
			$title_vertical_alignment = $meta_temp;
		} else {
			$title_vertical_alignment = target_qodef_options()->getOptionValue( 'title_area_vertial_alignment' );;
		}
...
if ( ! function_exists( 'target_qodef_title_area_background' ) ) {
	/**
	 * Function that returns title background style be applied in template
	 **/

	function target_qodef_title_area_background() {
		$id                           = target_qodef_get_page_id();
		$show_title_img               = true;
		...
		if ( ( $meta_temp = get_post_meta( $id, "qodef_title_area_background_color_meta", true ) ) != "" ) {
			...
			$background_color = target_qodef_options()->getOptionValue( 'title_area_background_color' );
		}
		...
		if ( ( $meta_temp = get_post_meta( $id, "qodef_title_area_background_image_meta", true ) ) != "" ) {
			...
			$background_image = target_qodef_options()->getOptionValue( 'title_area_background_image' );
		}

		//check for background image width
		$background_image_width = "";
		if ( $background_image !== '' ) {
			$background_image_width_dimensions_array = target_qodef_get_image_dimensions( $background_image );
