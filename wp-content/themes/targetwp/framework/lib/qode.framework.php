...
class TargetQodefFramework {
	...
	private function __construct() {
		$this->qodeOptions   = TargetQodefOptions::get_instance();
		$this->qodeMetaBoxes = TargetQodefMetaBoxes::get_instance();
	}
	...
	public function setSkin( TargetQodefSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class TargetQodefSkinManager
 *
...
/**
 * Class TargetQodefSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class TargetQodefSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( target_qodef_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
        <script>
            window.qodeSCIcon = '<?php echo target_qodef_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
...
class TargetQodefOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $target_qodef_options ) ) {
			return $target_qodef_options[ $key ];
...
/*
   Class: TargetQodefAdminPage
   A class that initializes Qode Admin Page
*/

class TargetQodefAdminPage implements iTargetQodefLayoutNode {
	...
	function __construct( $slug = "", $title_label = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new TargetQodefFieldFactory();
		$this->slug    = $slug;
	...
	public function renderChild( iTargetQodefRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: TargetQodefMetaBoxes
...
/*
   Class: TargetQodefMetaBox
   A class that initializes Qode Meta Box
*/

class TargetQodefMetaBox implements iTargetQodefLayoutNode {
	...
	public function setTitle( $label ) {
		...
		return $target_qodef_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

if ( ! function_exists( 'target_qodef_init_framework_variable' ) ) {

	function target_qodef_init_framework_variable() {

		global $target_qodef_Framework;

		$target_qodef_Framework = TargetQodefFramework::get_instance();
		$qodeSkinManager        = new TargetQodefSkinManager();
		$target_qodef_Framework->setSkin( $qodeSkinManager->getSkin() );
	}

	add_action( 'target_qodef_before_options_map', 'target_qodef_init_framework_variable' );
}
?>