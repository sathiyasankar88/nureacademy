...
if ( ! empty( $row_type ) && $row_type == 'parallax' ) {
	...
	if ( target_qodef_options()->getOptionValue( 'parallax_on_off' ) == 'off' ) {
...
if ( $row_type == 'row' && $css_animation != '' ) {
	...
	if ( $transition_delay !== '' ) {
		...
		$inner_start        .= '<div ' . target_qodef_get_inline_style( $animation_styles ) . '>';
		$inner_end          .= '</div>';
	} else {
		$inner_start .= '<div>';
		$inner_end   .= '</div>';
...
if ( $video == 'show_video' ) {
	...
	if ( $video_image ) {
		$video_output .= '<div class="qodef-mobile-video-image" ' . target_qodef_get_inline_attr( $video_mobile_style, 'style' ) . '></div>';
	}
	$video_output .= '<div ' . target_qodef_get_class_attribute( $video_overlay_class ) . target_qodef_get_inline_attr( $video_overlay_style, 'style' ) . '></div>';
	$video_output .= '<div class="qodef-video-wrap">';
	$video_output .= '<video class="qodef-video" width="1920" height="800" ' . target_qodef_get_inline_attr( $video_attrs, 'poster' ) . ' controls="controls" preload="auto" loop autoplay muted>';
	if ( ! empty( $video_webm ) ) {
	...
	if ( $v_image ) {
		$video_output .= '<img ' . target_qodef_get_inline_attr( $v_image, 'src' ) . ' width="1920" height="800" title="' . esc_attr__( 'No video playback capabilities', 'targetwp' ) . '" alt="' . esc_attr__( 'Video thumb', 'targetwp' ) . '" />';
	}
	$video_output .= '</object>';
	$video_output .= '</video>';
	$video_output .= '</div>';
...
print target_qodef_get_module_part( $output );